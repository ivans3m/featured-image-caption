...
class FOAFIC_Featured_Image_Caption {
    ...
    public function enqueue_block_editor_assets() {
        ...
        $js_file = FOAFIC_PLUGIN_PATH . 'assets/editor.js';
        $js_version = file_exists($js_file) ? filemtime($js_file) : FOAFIC_VERSION;
        
        wp_enqueue_script(
            'foafic-featured-image-caption-editor',
            FOAFIC_PLUGIN_URL . 'assets/editor.js',
            array('wp-blocks', 'wp-element', 'wp-editor', 'wp-components', 'wp-i18n', 'wp-hooks', 'wp-block-editor'),
            $js_version,
            true
        );
        
        // Add cache busting for CSS
        $css_file = FOAFIC_PLUGIN_PATH . 'assets/editor.css';
        $css_version = file_exists($css_file) ? filemtime($css_file) : FOAFIC_VERSION;
    ...
    public function enqueue_frontend_assets() {
        // Add cache busting with file modification time
        $css_file = FOAFIC_PLUGIN_PATH . 'assets/frontend.css';
        $css_version = file_exists($css_file) ? filemtime($css_file) : FOAFIC_VERSION;
    ...
    public function modify_featured_image_block($block_content, $block) {
        ...
        // Check for our internal flag (for templates)
            ...
            $show_caption = $block['attrs']['_foafic_show_caption'];
        }
